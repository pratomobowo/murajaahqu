  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 relative">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 pb-32">
        <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100 text-center mb-8 relative overflow-hidden">
          ...
          {question.options.map((option) => {
             ...
             return (
              <button
                ...
                disabled={isAnswered}
                className={`relative w-full p-5 rounded-2xl border-2 text-lg font-medium transition-all duration-200 shadow-sm active:scale-[0.98] ${buttonStyle}`}
              >
      ...
      {isAnswered && (
        <div className="fixed bottom-20 left-0 right-0 z-40 mx-auto max-w-md px-6">
          <button
            ...
            className={`w-full py-4 rounded-2xl font-bold text-white shadow-lg shadow-primary-500/30 transition-all active:scale-[0.98] flex items-center justify-center gap-2 ${
              isCorrect ? 'bg-primary-600 hover:bg-primary-700' : 'bg-slate-800 hover:bg-slate-900'
            }`}
          >
